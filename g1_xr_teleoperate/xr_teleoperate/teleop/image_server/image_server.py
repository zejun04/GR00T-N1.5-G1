...
class SimulationCamera():
    def __init__(self, camera_name, img_shape, fps):
        """
        camera_name: 'front_camera', 'left_wrist_camera', 'right_wrist_camera'
        ...
        # 尝试连接到仿真环境的相机数据
        self._try_connect_to_simulation()

    def _try_connect_to_simulation(self):
        """尝试连接到Isaac Lab仿真环境的相机数据"""
        try:
            # 这里需要实现从仿真环境获取相机数据的逻辑
            # 暂时创建一个空的测试图像
            self.test_image = np.zeros((self.img_shape[0], self.img_shape[1], 3), dtype=np.uint8)
            self.is_connected = True
            logger_mp.info(f"[Image Server] Simulation camera {self.camera_name} initialized with test image")
        except Exception as e:
            logger_mp.error(f"[Image Server] Failed to initialize simulation camera {self.camera_name}: {e}")
            self.is_connected = False
    ...
    def get_frame(self):
        ...
        # 这里需要实现从仿真环境获取实时相机数据的逻辑
        # 暂时返回测试图像
        return self.test_image.copy()

    def release(self):
        self.is_connected = False
        logger_mp.info(f"[Image Server] Simulation camera {self.camera_name} released")
